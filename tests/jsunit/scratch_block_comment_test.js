...
var block;
var workspace;
var originalCreateIcon = Blockly.HappycodeBlockComment.createIcon;

function happycodeBlockCommentTest_setUp() {
  ...
  Blockly.HappycodeBlockComment.prototype.createIcon = function () {};
}

function happycodeBlockCommentTest_tearDown() {
  workspace.dispose();
  block.dispose();

  // Restore original createIcon function
  Blockly.HappycodeBlockComment.prototype.createIcon = originalCreateIcon;
}

function test_blockWithNoBlockComments() {
  happycodeBlockCommentTest_setUp();
  try {
    ...
    happycodeBlockCommentTest_tearDown();
  }
}
...
function test_createBlockCommentMinimalArguments() {
  happycodeBlockCommentTest_setUp();
  try {
    var comment = new Blockly.HappycodeBlockComment(block, 'Some comment text');
    assertEquals('Workspace has a block.', 1, workspace.getTopBlocks(false).length);
...
function test_createBlockCommentAllArguments() {
  happycodeBlockCommentTest_setUp();
  try {
    var comment = new Blockly.HappycodeBlockComment(block, 'Some comment text', 'aMockComment', 10, 20, true);
    assertEquals('Workspace has a block.', 1, workspace.getTopBlocks(false).length);
    assertEquals('Workspace has a comment.', 1, workspace.getTopComments(false).length);
    assertEquals('Comment knows about workspace.', workspace, comment.workspace);
  } finally {
...
function test_blockCommentXYWhenPositionProvided() {
  happycodeBlockCommentTest_setUp();
  try {
    var comment = new Blockly.HappycodeBlockComment(block, 'Some comment text', 'aMockComment', 10, 20);
...
function test_blockCommentXYWhenPositionNotProvided() {
  happycodeBlockCommentTest_setUp();
  try {
    var comment = new Blockly.HappycodeBlockComment(

...
/**
 * @fileoverview Utility methods for Happycode Blocks but not Blockly.
 * @author fenichel@google.com (Rachel Fenichel)
 */
'use strict';

/**
 * @name Blockly.happycodeBlocksUtils
 * @namespace
 **/
goog.provide('Blockly.happycodeBlocksUtils');


/**
 * Measure some text using a canvas in-memory.
 * Does not exist in Blockly, but needed in happycode-blocks
 ...
 * @package
 */
Blockly.happycodeBlocksUtils.measureText = function(fontSize, fontFamily,
    fontWeight, text) {
 ...
 * @package
 */
Blockly.happycodeBlocksUtils.encodeEntities = function(rawStr) {
 ...
 * Happycode specific to help the VM handle deleting obscured shadows.
 * @param {Blockly.Block} block the root block to be processed.
 * @package
 */
Blockly.happycodeBlocksUtils.changeObscuredShadowIds = function(block) {
 ...
 * blocks have special behaviour in happycode-blocks: they're duplicated when
 ...
 * @package
 */
Blockly.happycodeBlocksUtils.isShadowArgumentReporter = function(block) {
 ...
 */
Blockly.happycodeBlocksUtils.compareStrings = function(str1, str2) {
 ...
 * @package
 */
Blockly.happycodeBlocksUtils.blockIsRecyclable = function(block) {
  ...
  for (var i = 0; i < block.inputList.length; i++) {
    ...
    if (input.connection) {
      ...
      if (child && !Blockly.happycodeBlocksUtils.blockIsRecyclable(child)) {
        return false;
      }
    }
  }
 ...
 * option in Happycode Blocks.  The block is duplicated and attached to the mouse,
 ...
 * @package
 */
Blockly.happycodeBlocksUtils.duplicateAndDragCallback = function(oldBlock, event) {
  ...
  return function(e) {
    ...
    setTimeout(function() {
      ...
      // positioned and has had its shadow IDs fixed (Happycode-specific).
      ...
      try {
        ...
        // Happycode-specific: Give shadow dom new IDs to prevent duplicating on paste
        Blockly.happycodeBlocksUtils.changeObscuredShadowIds(newBlock);

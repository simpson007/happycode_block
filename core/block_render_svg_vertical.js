...
goog.require('Blockly.happycodeBlocksUtils');
...
Blockly.BlockSvg.prototype.updateColour = function() {
  ...
  var renderShadowed = this.isShadow() &&
      !Blockly.happycodeBlocksUtils.isShadowArgumentReporter(this);
...
Blockly.BlockSvg.prototype.render = function(opt_bubble) {
  ...
  var icons = this.getIcons();
  var happycodeCommentIcon = null;
  for (var i = 0; i < icons.length; i++) {
    if (icons[i] instanceof Blockly.HappycodeBlockComment) {
      // Don't render happycode block comment icon until
      // after the inputs
      happycodeCommentIcon = icons[i];
    } else {
  ...
  if (this.isHappycodeExtension && this.outputConnection) {
    cursorX += this.RTL ?
  ...
  // Position the Happycode Block Comment Icon at the end of the block
  if (happycodeCommentIcon) {
    var iconX = this.RTL ? -inputRows.rightEdge : inputRows.rightEdge;
    var inputMarginY = inputRows[0].height / 2;
    happycodeCommentIcon.renderIcon(iconX, inputMarginY);
  }
...
Blockly.BlockSvg.prototype.renderFields_ = function(fieldList, cursorX,
  ...
  for (var t = 0, field; field = fieldList[t]; t++) {
    ...
    if (this.isHappycodeExtension && (field === this.inputList[0].fieldRow[0])
...
Blockly.BlockSvg.prototype.computeInputHeight_ = function(input, row,
  ...
  if (this.inputList.length === 1 && this.outputConnection &&
    ...
    // make it taller.
    if (this.isHappycodeExtension && this.previousConnection) {
...
Blockly.BlockSvg.prototype.renderDrawLeft_ = function(steps) {
  if (this.outputConnection) {
    // Happycode-style reporters have output connection y at half block height.

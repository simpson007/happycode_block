...
goog.provide('Blockly.HappycodeBlockComment');

goog.require('Blockly.Comment');
goog.require('Blockly.Events.BlockChange');
goog.require('Blockly.Events.Ui');
goog.require('Blockly.Icon');
goog.require('Blockly.HappycodeBubble');
 ...
 * @constructor
 */
Blockly.HappycodeBlockComment = function(block, text, id, x, y, minimized) {
  Blockly.HappycodeBlockComment.superClass_.constructor.call(this, block);
  /**
  ...
  if (!block.rendered) {
    Blockly.HappycodeBlockComment.fireCreateEvent(this);
  }
  // If the block is rendered, fire event the create event when the comment is made
  // visible
};
goog.inherits(Blockly.HappycodeBlockComment, Blockly.Comment);

/**
 * Width of bubble.
 * @private
 */
Blockly.HappycodeBlockComment.prototype.width_ = 200;

/**
 * Height of bubble.
 * @private
 */
Blockly.HappycodeBlockComment.prototype.height_ = 200;

/**
 * Comment Icon Size.
 * @package
 */
Blockly.HappycodeBlockComment.prototype.SIZE = 0;

/**
 * Offset for text area in comment bubble.
 * @private
 */
Blockly.HappycodeBlockComment.TEXTAREA_OFFSET = 12;
 ...
 * @private
 */
Blockly.HappycodeBlockComment.MAX_LABEL_LENGTH = 12;

/**
 * Maximum character length for comment text.
 * @private
 */
Blockly.HappycodeBlockComment.COMMENT_TEXT_LIMIT = 8000;

/**
 * Width that a minimized comment should have.
 * @private
 */
Blockly.HappycodeBlockComment.MINIMIZE_WIDTH = 200;
 ...
 * @private
 */
Blockly.HappycodeBlockComment.prototype.drawIcon_ = function(_group) {
  // NO-OP -- Don't render a comment icon for Happycode block comments
};
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.renderIcon = function(cursorX, topMargin) {
 ...
 * @private
 */
Blockly.HappycodeBlockComment.prototype.createEditor_ = function() {
  this.foreignObject_ = Blockly.utils.createSvgElement('foreignObject',
      {
        'x': Blockly.HappycodeBubble.BORDER_WIDTH,
        'y': Blockly.HappycodeBubble.BORDER_WIDTH + Blockly.HappycodeBubble.TOP_BAR_HEIGHT,
        'class': 'happycodeCommentForeignObject'
      },
      null);
  var body = document.createElementNS(Blockly.HTML_NS, 'body');
  body.setAttribute('xmlns', Blockly.HTML_NS);
  body.className = 'blocklyMinimalBody happycodeCommentBody';
  var textarea = document.createElementNS(Blockly.HTML_NS, 'textarea');
  textarea.className = 'happycodeCommentTextarea happycodeCommentText';
  textarea.setAttribute('dir', this.block_.RTL ? 'RTL' : 'LTR');
  textarea.setAttribute('maxlength', Blockly.HappycodeBlockComment.COMMENT_TEXT_LIMIT);
  textarea.setAttribute('placeholder', Blockly.Msg.WORKSPACE_COMMENT_DEFAULT_TEXT);
  body.appendChild(textarea);
  this.textarea_ = textarea;
  this.textarea_.style.margin = (Blockly.HappycodeBlockComment.TEXTAREA_OFFSET) + 'px';
 ...
 * @private
 */
Blockly.HappycodeBlockComment.prototype.textareaFocus_ = function(e) {
  Blockly.HappycodeBlockComment.superClass_.textareaFocus_.call(this, e);
 ...
 * @private
 */
Blockly.HappycodeBlockComment.prototype.resizeBubble_ = function() {
  if (this.isVisible() && !this.isMinimized_) {
    var size = this.bubble_.getBubbleSize();
    var doubleBorderWidth = 2 * Blockly.HappycodeBubble.BORDER_WIDTH;
    var textOffset = Blockly.HappycodeBlockComment.TEXTAREA_OFFSET * 2;
    this.foreignObject_.setAttribute('width', size.width - doubleBorderWidth);
    this.foreignObject_.setAttribute('height', size.height - doubleBorderWidth - Blockly.HappycodeBubble.TOP_BAR_HEIGHT);
    this.textarea_.style.width = (size.width - textOffset) + 'px';
    this.textarea_.style.height = (size.height - doubleBorderWidth -
       Blockly.HappycodeBubble.TOP_BAR_HEIGHT - textOffset) + 'px';
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.updateColour = function() {
  if (this.isVisible()) {
 ...
 * @private
 */
Blockly.HappycodeBlockComment.prototype.autoPosition_ = function() {
  ...
  if (this.isMinimized_) {
    ...
    this.y_ = this.iconXY_.y - (Blockly.HappycodeBubble.TOP_BAR_HEIGHT / 2);
  } else {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.setVisible = function(visible) {
  ...
  if (visible) {
    ...
    // Create the bubble.
    this.bubble_ = new Blockly.HappycodeBubble(
 ...
 * @private
 */
Blockly.HappycodeBlockComment.prototype.toggleMinimize_ = function() {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.setMinimized = function(minimize) {
  ...
  if (minimize) {
    ...
    this.setBubbleSize(Blockly.HappycodeBlockComment.MINIMIZE_WIDTH,
        Blockly.HappycodeBubble.TOP_BAR_HEIGHT);
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.setBubbleSize = function(width, height) {
  if (this.bubble_) {
    if (this.isMinimized_) {
      this.bubble_.setBubbleSize(Blockly.HappycodeBlockComment.MINIMIZE_WIDTH,
          Blockly.HappycodeBubble.TOP_BAR_HEIGHT);
    } else {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.setSize = function(width, height) {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.getLabelText = function() {
  if (this.text_.length > Blockly.HappycodeBlockComment.MAX_LABEL_LENGTH) {
    if (this.block_.RTL) {
      return '\u2026' + this.text_.slice(0, Blockly.HappycodeBlockComment.MAX_LABEL_LENGTH);
    }
    return this.text_.slice(0, Blockly.HappycodeBlockComment.MAX_LABEL_LENGTH) + '\u2026';
  } else {
    return this.text_;
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.setText = function(text) {
  if (this.text_ != text) {
    Blockly.Events.fire(new Blockly.Events.CommentChange(
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.moveTo = function(x, y) {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.getXY = function() {
  if (this.bubble_) {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.getHeightWidth = function() {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.getBoundingRectangle = function() {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.isMinimized = function() {
 ...
 * @private
 */
Blockly.HappycodeBlockComment.prototype.showContextMenu_ = function(e) {
  var menuOptions = [];
 ...
 * @package
 */
Blockly.HappycodeBlockComment.prototype.toXmlWithXY = function() {
 ...
 * @package
 */
Blockly.HappycodeBlockComment.fireCreateEvent = function(comment) {
 ...
 */
Blockly.HappycodeBlockComment.prototype.dispose = function() {
  if (Blockly.Events.isEnabled()) {
 ...
 */
Blockly.HappycodeBlockComment.prototype.focus = function() {

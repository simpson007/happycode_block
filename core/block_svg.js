 ...
 * @param {number=} commentX Optional x position for happycode comment in workspace coordinates
 * @param {number=} commentY Optional y position for happycode comment in workspace coordinates
 * @param {boolean=} minimized Optional minimized state for happycode comment, defaults to false
 */
Blockly.BlockSvg.prototype.setCommentText = function(text, commentId,
  ...
  if (goog.isString(text)) {
    if (!this.comment) {
      this.comment = new Blockly.HappycodeBlockComment(this, text, commentId,
          commentX, commentY, minimized);
      changedState = true;
    } else {

...
Blockly.Xml.blockToDom = function(block, opt_noId) {
  ...
  Blockly.Xml.happycodeCommentToDom_(block, element);
...
/**
 * Encode a HappycodeBlockComment as XML.
 * @param {!Blockly.HappycodeBlockComment} block The block possibly containing
 *     a comment to encode.
 * @param {!Element} element The XML element to which the comment should
 *     encoding should be attached.
 * @private
 */
Blockly.Xml.happycodeCommentToDom_ = function(block, element) {
  ...
  if (commentText) {
    ...
    if (typeof block.comment == 'object') {
      ...
      var hw;
      if (block.comment instanceof Blockly.HappycodeBlockComment) {
  ...
  for (var i = 0, xmlChild; xmlChild = xmlBlock.childNodes[i]; i++) {
    ...
    switch (xmlChild.nodeName.toLowerCase()) {
      ...
      case 'comment':
        ...
        // Note bubbleX and bubbleY can be NaN, but the HappycodeBlockComment

...
// File paths
const PATH_INPUT = path.resolve(__dirname, '../msg/happycode_msgs.js');

// Match lines of the happycode_msgs file
// Blockly.HappycodeMsgs.locales indicates the start of a new locale
...
const match = function (str) {
    if (str.indexOf('Blockly.HappycodeMsgs.locales') !== 0) return true;
...
const validateKeys = function () {
    ...
    assert.strictEqual(keys.length, Object.keys(en).length, `happycode_msgs-${locale}: number of keys doesn't match`);
    keys.map(item => assert(enKeys.includes(item), `happycode_msgs-${locale}: has key ${item} not in en`));
    enKeys.map(item => assert(keys.includes(item), `happycode_msgs-${locale}: is missing key ${item}`));
}

...
/**
 * @fileoverview
 * Script to pull translations for blocks from transifex and generate the happycode_msgs file.
...
const locales = require('happycode-l10n').default;

// Globals
const PATH_OUTPUT = path.resolve(__dirname, '../msg');
const PROJECT = 'happycode-editor'
...
goog.provide('Blockly.HappycodeMsgs.allLocales');

goog.require('Blockly.HappycodeMsgs');
...
Promise.all(Object.keys(locales).map(getLocaleData)).then(function (values) {
  values.forEach(function (translation) {
    ...
    file += '\n';
    file += `Blockly.HappycodeMsgs.locales["${translation.locale}"] =\n`;
    file += JSON.stringify(translation.translations, null, 4);
    file += ';\n';
  });
  file += '// End of combined translations\n';
  // write combined file
  fs.writeFileSync(`${PATH_OUTPUT}/happycode_msgs.js`, file);
}).catch((err) => {
  console.error(err);
  process.exit(1);
});
